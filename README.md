...
工具调用、结果与最终回答之间的关系。
运行状态面板会持续显示当前阶段、事件数、工具调用数、token、缓存命中率、耗时和 Run ID；
失败时保留错误摘要。
...
`text_delta` 表示文本片段，不保证等于一个 token，也不会写入 Trace。当前内置 Provider 尚未
接入原生流，只会在模型完整响应后产生后续事件。
...
- [ ] 内置 Provider 原生文本流实现
- [ ] 稳定多 Agent 编排；当前 `agent_as_tool` 不等同于完整编排系统
