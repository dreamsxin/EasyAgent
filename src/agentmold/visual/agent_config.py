"""Agent construction and tool loading for the visual lab.
...
def bind_visual_tools(
    ...
    *,
    ...
    """Create session-owned Tool bindings without mutating source tools."""
    ...
    for tool_name in selected_tools:
        source = available_tools[tool_name]
        description = overrides.get(tool_name, "").strip() or source.description
        bound.append(
            Tool(
                func=source.func,
                name=source.name,
                ...
                or (require_approval and origins.get(tool_name, "").startswith("MCP")),
            )
        )
    return bound
...
def build_agent(
    ...
    *,
    ...
    ``log_level`` is a deprecated compatibility keyword. The visual lab always
    uses its own event views instead of writing provider activity to stdout.
    """
    del log_level
...
def visual_approval_gate(tool_name: str, arguments: dict[str, Any]) -> bool:
    """Safely refuse destructive tools in the synchronous visual lab.

    The run still emits an ``approval_request`` event, so students can inspect
    the human-in-the-loop boundary and its refused tool result. A resumable
    approve action would require a background run/state machine; ordinary
    Streamlit buttons cannot resume the currently suspended generator.
    """
...
def agent_signature(
    ...
    *,
    ...
    ``log_level`` is accepted for compatibility but intentionally omitted from
    the signature because visual stdout logging is always disabled.
    """
    del log_level
    return (
        name,
        ...
        tuple(
            sorted(
                (name, description)
                for name, description in (description_overrides or {}).items()
